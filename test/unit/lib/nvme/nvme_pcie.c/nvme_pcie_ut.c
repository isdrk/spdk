...
DEFINE_STUB(spdk_pci_device_get_interrupt_efd_by_index, int, (struct spdk_pci_device *dev,
		uint32_t index), 0);
DEFINE_STUB(spdk_pci_device_create_dmabuf, struct spdk_dmabuf *, (struct spdk_pci_device *spdk_dev,
		int bar), NULL);
DEFINE_STUB_V(spdk_pci_device_destroy_dmabuf, (struct spdk_pci_device *spdk_dev,
		struct spdk_dmabuf *));

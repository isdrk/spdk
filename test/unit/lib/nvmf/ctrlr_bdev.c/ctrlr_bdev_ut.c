...
{
	return spdk_bdev_is_md_interleaved(desc->bdev);
}
...
{
	return spdk_bdev_is_dif_head_of_md(desc->bdev);
}
...
nvmf_ns_get_rescap(struct spdk_nvmf_ns *ns)
{
	struct spdk_nvme_rescap rescap = {
		.persist = nvmf_ns_is_ptpl_capable(ns),
		.write_exclusive = 1,
		.exclusive_access = 1,
		.write_exclusive_reg_only = 1,
		.exclusive_access_reg_only = 1,
		.write_exclusive_all_reg = 1,
		.exclusive_access_all_reg = 1,
		.ignore_existing_key = 1,
	};
	return rescap;
}
...
test_nvmf_bdev_ctrlr_cmd(void)
{
	...
	rc = nvmf_bdev_ctrlr_compare_cmd(&bdev, &desc, &ch, &req);
	CU_ASSERT(rc == SPDK_NVMF_REQUEST_EXEC_STATUS_ASYNCHRONOUS);
	...
	rc = nvmf_bdev_ctrlr_flush_cmd(&bdev, &desc, &ch, &req);
	CU_ASSERT(rc == SPDK_NVMF_REQUEST_EXEC_STATUS_COMPLETE);
	CU_ASSERT(rsp.nvme_cpl.status.sct == SPDK_NVME_SCT_GENERIC);
	...
	rc = nvmf_bdev_ctrlr_copy_cmd(&bdev, &desc, &ch, &req);
	CU_ASSERT(rc == SPDK_NVMF_REQUEST_EXEC_STATUS_ASYNCHRONOUS);

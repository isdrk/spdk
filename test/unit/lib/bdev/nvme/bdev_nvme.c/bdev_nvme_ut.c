...
DEFINE_STUB(spdk_accel_append_copy, int, (struct spdk_accel_sequence **seq,
		struct spdk_io_channel *ch,
...
test_pending_reset(void)
{
	...
	CU_ASSERT(TAILQ_FIRST(&nvme_ctrlr->pending_resets) == second_bdev_io);
...
test_reset_bdev_ctrlr(void)
{
	...
	CU_ASSERT(nvme_ctrlr1->resetting == true);
	CU_ASSERT(nvme_ctrlr1->ctrlr_op_cb_arg == first_bio);
	CU_ASSERT(TAILQ_FIRST(&nvme_ctrlr1->pending_resets) == second_bdev_io);
...
test_bdev_reset_abort_io(void)
{
	...
	struct nvme_ctrlr_opts opts = {};
	struct spdk_nvme_ctrlr *ctrlr;
	struct spdk_nvme_ctrlr_opts dopts = {};
	struct nvme_ctrlr *nvme_ctrlr;
	const int STRING_SIZE = 32;
	const char *attached_names[STRING_SIZE];
	struct nvme_bdev *bdev;
...
test_race_between_clear_pending_resets_and_reset_ctrlr_complete(void)
{
	...
	CU_ASSERT(TAILQ_FIRST(&nvme_ctrlr->pending_resets) == bdev_io);

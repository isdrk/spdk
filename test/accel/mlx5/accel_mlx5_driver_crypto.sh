...
validate_crypto_umr_stats() {
	...
	encrypt_mkey=$(echo $stats | jq -r '.Total.tasks.encrypt_mkey')
	decrypt_mkey=$(echo $stats | jq -r '.Total.tasks.decrypt_mkey')
	if [ $encrypt_mkey == 0 ] || [ $decrypt_mkey == 0 ]; then
		echo "Unexpected number of encrypt: $encrypt_mkey or decrypt: $decrypt_mkey operations, expected > 0"
		return 1
	fi
	if [ $((encrypt_mkey + decrypt_mkey)) != $(echo $stats | jq -r '.Total.tasks.total') ]; then

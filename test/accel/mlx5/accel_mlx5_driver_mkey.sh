...
validate_mkey_stats() {
	...
	if [ $val != 0 ] && [ $val != $(echo $stats | jq -r '.total.tasks.total') ]; then
		echo "Unexpected number of mkey tasks: $val, expected > 0 and no other tasks"
		return 1
	fi
}
...
$bdevperf --json <(gen_accel_mlx5_driver_rdma_json) -q 64 -o 65536 -t 10 -w verify -M 50 -m 0xc -r $app_sock

...
static int bdev_read_submit_split(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
				  ...
				  bool has_metadata, uint8_t dma_path_id,
...
bdev_qos_config_json(struct spdk_bdev_qos *qos, struct spdk_json_write_ctx *w)
{
	...
	spdk_json_write_named_string(w, "method", "bdev_qos_create");
	...
	TAILQ_FOREACH(bdev, &qos->bdevs, internal.qos_link) {
		...
		spdk_json_write_named_string(w, "method", "bdev_qos_add_bdev");

		spdk_json_write_named_object_begin(w, "params");
		spdk_json_write_named_string(w, "name", qos->name);
		spdk_json_write_named_string(w, "bdev_name", bdev->name);
		spdk_json_write_object_end(w);
...
spdk_bdev_initialize(spdk_bdev_init_cb cb_fn, void *cb_arg)
{
	...
	rc = bdev_qos_modules_init();
	if (rc != 0) {
		SPDK_ERRLOG("bdev QoS modules init failed\n");
		bdev_init_complete(-1);
		return;
	}
	       ...
	       spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	...
	bdev_io->dma_path_id = dma_path_id;
		  ...
		  struct spdk_bdev_ext_io_opts *opts)
{
	...
	if (opts) {
		...
		dma_path_id = bdev_get_ext_io_opt(opts, dma_path_id, 0);
		if (md) {
	...
	bdev_rw_submit(desc, channel, bdev_io, iov, iovcnt, md_buf, offset_blocks, num_blocks,
		       domain, domain_ctx, seq, dif_check_flags, false, 0, 0, dma_path_id,
		       SPDK_BDEV_IO_TYPE_READ, cb, cb_arg);
		       ...
		       spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	...
	bdev_rw_submit(desc, channel, bdev_io, iov, iovcnt, md_buf, offset_blocks, num_blocks,
		       ...
		       0, 0, dma_path_id, SPDK_BDEV_IO_TYPE_READ, cb, cb_arg);
		   ...
		   struct spdk_bdev_ext_io_opts *opts)
{
	...
	bdev_rw_submit(desc, channel, bdev_io, iov, iovcnt, md_buf, offset_blocks, num_blocks,
		       ...
		       nvme_cdw12_raw, nvme_cdw13_raw, dma_path_id,
		       SPDK_BDEV_IO_TYPE_WRITE, cb, cb_arg);
...
uint8_t
spdk_bdev_io_get_dma_path_id(const struct spdk_bdev_io *bdev_io)
{
	if (bdev_io == NULL) {
		return 0;
	}
	return bdev_io->dma_path_id;
}

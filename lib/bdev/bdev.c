...
bdev_finish_unregister_bdevs_iter(void *cb_arg, int bdeverrno)
{
	...
	if (TAILQ_EMPTY(&g_bdev_mgr.bdevs)) {
		...
		/*
		 * Bdev module finish need to be deferred as we might be in the middle of
...
spdk_bdev_finish(spdk_bdev_fini_cb cb_fn, void *cb_arg)
{
	...
	spdk_io_device_unregister(&g_qos_mgr, NULL);
...
{
	bdev_qos_limits_cache_reset(&qos_ch->limits);
}
...
bdev_qos_channel_create(struct spdk_bdev_qos *qos)
{
	struct spdk_bdev_qos_channel *qos_ch;
	struct spdk_io_channel *pg_io_ch;

	qos_ch = calloc(1, sizeof(*qos_ch));
	if (!qos_ch) {
		return NULL;
	}
...
static void
bdev_qos_channel_destroy(struct spdk_bdev_qos_channel *qos_ch)
{
	struct spdk_bdev_qos_poll_group *group = qos_ch->group;
...
bdev_channel_destroy_resource(struct spdk_bdev_channel *ch)
{
	...
	if (ch->qos_ch != NULL) {
		...
		spdk_spin_lock(&bdev->internal.spinlock);
		bdev_qos_channel_destroy(ch->qos_ch);
		spdk_spin_unlock(&bdev->internal.spinlock);
	}
...
bdev_enable_qos(struct spdk_bdev *bdev, struct spdk_bdev_channel *ch)
{
	...
	if (qos) {
		if (!(ch->flags & BDEV_CH_QOS_ENABLED)) {
			ch->qos_ch = bdev_qos_channel_create(qos);
			if (ch->qos_ch == NULL) {
				return;
			}
		     ...
		     struct spdk_io_channel *ch, void *_ctx)
{
	...
	if (bdev_ch->flags & BDEV_CH_QOS_ENABLED) {
		assert(bdev_ch->qos_ch != NULL);

		bdev_qos_unblock_all_queued_io(bdev_ch);

		spdk_spin_lock(&bdev->internal.spinlock);
		bdev_qos_channel_destroy(bdev_ch->qos_ch);
		spdk_spin_unlock(&bdev->internal.spinlock);
		bdev_ch->qos_ch = NULL;
		    ...
		    struct spdk_io_channel *ch, void *_ctx)
{
	...
	if (bdev_ch->qos_ch != NULL) {
		bdev_qos_limits_cache_init(&bdev_ch->qos_ch->limits,
					   &bdev_ch->qos_ch->qos->limits);
	}

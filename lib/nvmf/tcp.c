			  ...
			  uint32_t data_offset, uint32_t data_len)
{
	...
	if (iovcnt == 1) {
		pdu->data_iov[0].iov_base = (void *)((uint64_t)iov[0].iov_base + buf_offset);
...
static inline int nvme_tcp_derive_tls_psk(const uint8_t *psk_in, uint64_t psk_in_size,
				...
				struct nvme_tcp_pdu *pdu)
{
	...
	if (!tcp_req) {
		 ...
		 * send another one.
		 */
		if (tqpair->state_cntr[TCP_REQUEST_STATE_TRANSFERRING_CONTROLLER_TO_HOST]) {
			return;
		}
...
nvme_tcp_read_payload_data(struct spdk_nvmf_tcp_qpair *tqpair, struct nvme_tcp_pdu *pdu)
{
	...
	ret = 0;
	for (i = 0; i < iovcnt; i++) {
		rc = nvme_tcp_read_data(tqpair, iov[i].iov_len, iov[i].iov_base);
		if (rc < 0) {
			return NVME_TCP_CONNECTION_FATAL;
		}
...
nvmf_tcp_sock_process(struct spdk_nvmf_tcp_qpair *tqpair)
{
	...
	do {
		...
		case NVME_TCP_PDU_RECV_STATE_AWAIT_PDU_PAYLOAD:
			...
			rc = nvme_tcp_read_payload_data(tqpair, pdu);
			if (rc < 0) {
				nvmf_tcp_qpair_set_recv_state(tqpair, NVME_TCP_PDU_RECV_STATE_QUIESCING);
				break;
			}
...
nvmf_tcp_opts_init(struct spdk_nvmf_transport_opts *opts)
{
	...
	opts->small_buf_cache_size =	SPDK_NVMF_TCP_DEFAULT_SMALL_BUFFER_CACHE_SIZE;
	opts->large_buf_cache_size =	SPDK_NVMF_TCP_DEFAULT_LARGE_BUFFER_CACHE_SIZE;

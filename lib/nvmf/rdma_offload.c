...
#define TRACE_RDMA_OFFLOAD_REQUEST_STATE_NEED_ACCEL_TASK			SPDK_TPOINT_ID(TRACE_GROUP_NVMF_RDMA_OFFLOAD, 0x13)
...
nvmf_rdma_offload_trace(void)
{
	...
	spdk_trace_register_description("RDMA_REQ_NEED_ACCEL",
					TRACE_RDMA_OFFLOAD_REQUEST_STATE_NEED_ACCEL_TASK,
					OWNER_TYPE_NONE, OBJECT_NVMF_RDMA_OFFLOAD_IO, 0,
					SPDK_TRACE_ARG_TYPE_PTR, "qpair");
...
nvmf_rdma_qpair_initialize(struct spdk_nvmf_qpair *qpair)
{
	...
	qp_init_attr.domain_transfer	= rqpair->start_accel_sequence ?
					  nvmf_rdma_memory_domain_transfer_data : NULL;
...
request_transfer_out(struct spdk_nvmf_request *req, int *data_posted)
{
	...
	*data_posted = 0;
	...
	if (spdk_unlikely(rsp->status.sc != SPDK_NVME_SC_SUCCESS)) {
		 ...
		 */
		rdma_req->num_outstanding_data_wr = 0;
	} else if (req->xfer == SPDK_NVME_DATA_CONTROLLER_TO_HOST && !rdma_req->data_transferred) {
	...
	SPDK_DEBUGLOG(rdma_offload, "sent req %p, data_posted %d, outstanding_wrs %u\n", rdma_req,
		      *data_posted, rdma_req->num_outstanding_data_wr);
...
static int
request_transfer_out_data(struct spdk_nvmf_request *req)
{
	struct spdk_nvmf_rdma_request	*rdma_req;
	struct spdk_nvmf_rdma_qpair	*rqpair;
	struct ibv_send_wr		*first = NULL;
	...
	SPDK_DEBUGLOG(rdma_offload, "sent data for req %p, outstanding_wrs %u\n", rdma_req,
...
nvmf_rdma_connect(struct spdk_nvmf_transport *transport, struct rdma_cm_event *event)
{
	...
	if (private_data->qid == 0) {
		...
		rqpair->start_accel_sequence = false;
...
nvmf_rdma_req_finish_data_transfer(struct spdk_nvmf_rdma_request *rdma_req, int rc)
{
	...
	SPDK_DEBUGLOG(rdma_offload, "req %p, finish data transfer, rc %d\n", rdma_req, rc);
				     ...
				     uint32_t send_flags)
{
	...
	if (wr->next == NULL || wr->next == &rdma_req->rsp.wr) {
		...
		wr->sg_list[0].addr = 0;
		wr->sg_list[0].length = rdma_req->common.req.length;
		...
		inline_segment = &rdma_req->common.req.cmd->nvme_cmd.dptr.sgl1;
		num_sgl_descriptors = inline_segment->unkeyed.length / sizeof(struct spdk_nvme_sgl_descriptor);
		desc = (struct spdk_nvme_sgl_descriptor *)rdma_req->recv->buf + inline_segment->address;
...
static int
nvmf_rdma_memory_domain_transfer_data(struct spdk_memory_domain *dst_domain, void *dst_domain_ctx,
				      struct iovec *dst_iov, uint32_t dst_iovcnt,
				      struct spdk_memory_domain *src_domain, void *src_domain_ctx,
				      struct iovec *src_iov, uint32_t src_iovcnt,
				      struct spdk_memory_domain_translation_result *translation,
				      spdk_memory_domain_data_cpl_cb cpl_cb, void *cpl_cb_arg)
{
	struct spdk_nvmf_rdma_request *rdma_req = dst_domain_ctx;
	...
	if (rdma_req->common.req.xfer == SPDK_NVME_DATA_HOST_TO_CONTROLLER) {
		...
		SPDK_DEBUGLOG(rdma_offload, "req %p, lkey %u, transfer H2C\n", rdma_req, translation->rdma.lkey);
		 ...
		 * the media */
		assert(rdma_req->common.req.data_from_pool);
		...
		if (translation->size >= offsetof(struct spdk_memory_domain_translation_result,
			...
			last->next = NULL;
			SPDK_DEBUGLOG(rdma_offload, "req %p, lkey %u, transfer C2H\n", rdma_req, translation->rdma.lkey);
				      ...
				      struct spdk_nvmf_rdma_request *rdma_req)
{
	...
	opc = rdma_req->common.req.cmd->nvme_cmd.opc;
	if (spdk_unlikely(opc == SPDK_NVME_OPC_FABRIC || !rqpair->common.qpair.ctrlr ||
			  !rqpair->common.qpair.ctrlr->subsys)) {
		return false;
	}
	if (opc != SPDK_NVME_OPC_READ && opc != SPDK_NVME_OPC_WRITE) {
		return false;
	}
	subsys = rqpair->common.qpair.ctrlr->subsys;
	ns = _nvmf_subsystem_get_ns(subsys, rdma_req->common.req.cmd->nvme_cmd.nsid);
	if (spdk_unlikely(!ns)) {
		return false;
	}

	SPDK_DEBUGLOG(rdma_offload, "rqpair %p id %u, req %p, accel_seq %s\n", rqpair,
		      rqpair->common.qpair.qid,
		      rdma_req, ns->accel_sequence ? "YES" : "NO");
				   ...
				   struct spdk_nvmf_rdma_request *rdma_req)
{
	...
	rgroup = rqpair->poller->group;
	SPDK_DEBUGLOG(rdma_offload,
		      "req %p, offset %"PRIu64" opc %d append copy, buf %p len %zu, count %u\n",
		      rdma_req,
		      from_le64(&rdma_req->common.req.cmd->nvme_cmd.cdw10),
		      rdma_req->common.req.cmd->nvme_cmd.opc,
		      rdma_req->common.req.iov[0].iov_base,
		      rdma_req->common.req.iov[0].iov_len,
		      rdma_req->common.req.iovcnt);
	 ...
	 * during RDMA_READ operation. Once transfer_in completes, bdev layer writes data to
	 * the media */
	assert(rdma_req->common.req.accel_sequence == NULL);
	rc = spdk_accel_append_copy(&rdma_req->common.req.accel_sequence, rgroup->accel_ch,
	...
	if (spdk_unlikely(rc)) {
		...
		rsp = &rdma_req->common.req.rsp->nvme_cpl;
		...
		SPDK_DEBUGLOG(rdma_offload, "req %p, accel task failed, rc %d\n", rdma_req, rc);
		return rc;
	}
			  ...
			  struct spdk_nvmf_rdma_request *rdma_req)
{
	...
	do {
		...
		case RDMA_REQUEST_STATE_NEED_BUFFER:
			...
			break;
		case RDMA_REQUEST_STATE_NEED_ACCEL_TASK:
			spdk_trace_record(TRACE_RDMA_OFFLOAD_REQUEST_STATE_NEED_ACCEL_TASK, 0, 0,
					  (uintptr_t)rdma_req, (uintptr_t)rqpair);
		...
		case RDMA_REQUEST_STATE_DATA_TRANSFER_TO_HOST_PENDING:
			...
			rsp_needed = rdma_req->transfer_cpl_cb == NULL;

			if (rdma_req != STAILQ_FIRST(&rqpair->pending_rdma_write_queue)) {
				/* This request needs to wait in line to perform RDMA */
				break;
			}
			if ((rqpair->current_send_depth + rdma_req->num_outstanding_data_wr + rsp_needed) >
		...
		case RDMA_REQUEST_STATE_READY_TO_COMPLETE:
			...
			if (!rdma_req->transfer_cpl_cb) {
				...
				SPDK_DEBUGLOG(rdma_offload, "req %p, transfer data\n", rdma_req);
				rc = request_transfer_out_data(&rdma_req->common.req);
				assert(rc == 0); /* No good way to handle this currently */
				if (spdk_unlikely(rc)) {
					rdma_req->state = RDMA_REQUEST_STATE_COMPLETED;
				} else {
					rdma_req->state = RDMA_REQUEST_STATE_TRANSFERRING_CONTROLLER_TO_HOST;
				}
			}
			break;
...
nvmf_rdma_request_complete(struct spdk_nvmf_rdma_request *rdma_req)
{
	...
	assert(rdma_req->common.req.accel_sequence == NULL || rqpair->start_accel_sequence);
	rdma_req->common.req.accel_sequence = NULL;
	if (spdk_unlikely(rqpair->ibv_in_error_state)) {
		      ...
		      struct spdk_nvmf_rdma_poller *rpoller)
{
	...
	for (i = 0; i < reaped; i++) {
		...
		case RDMA_WR_TYPE_SEND:
			...
			rdma_req->num_outstanding_data_wr = 0;
			SPDK_DEBUGLOG(rdma_offload, "req %p, C2H completed, transfer_cpl_cb %p\n", rdma_req,
				      rdma_req->transfer_cpl_cb);
			...
			if (spdk_likely(!wc[i].status)) {
				if (wc[i].opcode == IBV_WC_RDMA_READ) {
					...
					if (rdma_req->num_outstanding_data_wr == 0) {
						...
						SPDK_DEBUGLOG(rdma_offload, "req %p, transfer_cpl_cb %p\n", rdma_req, rdma_req->transfer_cpl_cb);
						if (rdma_req->transfer_cpl_cb) {
					...
					/* That is only possible as part of async data transfer */
					assert(wc[i].opcode == IBV_WC_RDMA_WRITE);
					assert(rdma_req->transfer_cpl_cb);
					if (rdma_req->num_outstanding_data_wr == 0) {
						/* We need to wait for bdev layer to call req_complete */
						SPDK_DEBUGLOG(rdma_offload, "req %p, finish data transfer\n", rdma_req);

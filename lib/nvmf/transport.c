			 ...
			 bool named)
{
	...
	spdk_json_write_named_uint32(w, "small_buf_cache_size", opts->small_buf_cache_size);
...
#define SET_FIELD(field) \
	...
	SET_FIELD(small_buf_cache_size);
	...
	SET_FIELD(min_kato);
	SET_FIELD(kas);
	SET_FIELD(large_buf_cache_size);
...
{
	return transport->opts.small_buf_cache_size || transport->opts.large_buf_cache_size;
}
		      ...
		      spdk_nvmf_transport_create_done_cb cb_fn, void *cb_arg, bool sync)
{
	...
	if (ctx->opts.small_buf_cache_size != UINT32_MAX && ctx->opts.small_buf_cache_size != 0) {
		if (ctx->opts.small_buf_cache_size * spdk_env_get_core_count() > opts_iobuf.small_pool_count) {
			SPDK_WARNLOG("per core small_buf_cache_size %u is larger than iobuf small buffer count %" PRIu64
				     ", limiting to 50%% of the pool\n",
				     ctx->opts.small_buf_cache_size * spdk_env_get_core_count(), opts_iobuf.small_pool_count);
			ctx->opts.small_buf_cache_size = opts_iobuf.small_pool_count / spdk_env_get_core_count() / 2;
		}
	}
	if (ctx->opts.large_buf_cache_size != UINT32_MAX && ctx->opts.large_buf_cache_size != 0) {
		if (ctx->opts.large_buf_cache_size * spdk_env_get_core_count() > opts_iobuf.large_pool_count) {
			SPDK_WARNLOG("per core large_buf_cache_size %u is larger than iobuf large buffer count %" PRIu64
				     ", limiting to 50%% of the pool\n",
				     ctx->opts.large_buf_cache_size * spdk_env_get_core_count(), opts_iobuf.large_pool_count);
			ctx->opts.large_buf_cache_size = opts_iobuf.large_pool_count / spdk_env_get_core_count() / 2;
		}
	}
				 ...
				 struct spdk_nvmf_poll_group *group)
{
	...
	small_cache_size = transport->opts.small_buf_cache_size;

	/* small_buf_cache_size of UINT32_MAX means the value should be calculated dynamically
	...
	if (small_cache_size == UINT32_MAX) {
		...
		small_cache_size = (opts_iobuf.small_pool_count / 2) / num_poll_groups;
	}

	large_cache_size = transport->opts.large_buf_cache_size;

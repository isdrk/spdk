...
nvmf_rdma_log_wc_status(struct spdk_nvmf_rdma_qpair *rqpair, struct ibv_wc *wc)
{
	...
	if (wc->status == IBV_WC_WR_FLUSH_ERR) {
		...
		SPDK_DEBUGLOG(rdma,
			      "Error on CQ %p, (qp state %d, in_error %d) request 0x%lx, type %s, status: (%d): %s; vendor_err: 0x%04x\n",
		...
		SPDK_ERRLOG("Error on CQ %p, (qp state %d, in_error %d) request 0x%lx, type %s, status: (%d): %s, vendor_err: 0x%04x\n",
			    rqpair->poller->cq, rqpair->qpair.state, rqpair->ibv_in_error_state, wc->wr_id,
			    nvmf_rdma_wr_type_str(wr_type), wc->status, ibv_wc_status_str(wc->status),
			    wc->vendor_err);
	}

...
{
	assert(key != NULL && key_len > 0 && key_len <= 16);

	cmd->cdw11_bits.kv.kl = key_len;

	memcpy((uint8_t *)&cmd->rsvd2, key, spdk_min(key_len, 8));
		      ...
		      spdk_nvme_cmd_cb cb_fn, void *cb_arg, bool host_to_ctrlr)
{
	...
	if (key == NULL || key_len == 0 || key_len > 16 || data == NULL || data_len == 0) {
		return -EINVAL;
	}
			 ...
			 spdk_nvme_cmd_cb cb_fn, void *cb_arg)
{
	...
	if (key == NULL || key_len == 0 || key_len > 16) {
		return -EINVAL;
	}
		  ...
		  spdk_nvme_cmd_cb cb_fn, void *cb_arg)
{
	...
	if (start_key == NULL) {
		...
		if (start_key_len == 0 || start_key_len > 16) {
			return -EINVAL;
		}
		key = start_key;
	...
	req = nvme_allocate_request_user_copy(qpair, buffer, buffer_len, cb_fn, cb_arg, false);
	if (req == NULL) {
		return -ENOMEM;
	}

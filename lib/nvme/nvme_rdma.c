...
nvme_rdma_qpair_release_poller(struct nvme_rdma_qpair *rqpair)
{
	...
	if (poller->srq) {
		...
		poller->required_num_wc -= num_wc_decrement;
	}
			       ...
			       struct nvme_request *req)
{
	...
	if (req->accel_sequence ||
	    (rqpair->append_copy && req->payload.payload_size != 0)) {
				...
				struct nvme_rdma_qpair *rqpair)
{
	...
	if (rqpair->num_outstanding_reqs == 0 && STAILQ_EMPTY(&qpair->queued_req)) {
		...
		assert(group->num_active_qpairs > 0);
		group->num_active_qpairs--;
	}
}
		...
		uint32_t completions_per_qpair, spdk_nvme_disconnected_qpair_cb disconnected_qpair_cb)
{
	...
	completions_allowed = completions_per_qpair * group->num_active_qpairs;

...
SPDK_STATIC_ASSERT(sizeof(struct spdk_sock_request) == 80, "Incorrect size.");
 ...
 *
 * A pointer to this structure will be used by spdk_sock_listen() or spdk_sock_connect() to
 ...
 */
struct spdk_sock *spdk_sock_connect_async(const char *ip, int port, struct spdk_sock_opts *opts,

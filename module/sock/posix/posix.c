		  ...
		  struct spdk_sock_opts *opts)
{
	...
	sock = _posix_sock_listen(ip, port, opts, false);
	if (sock == NULL) {
		return sock;
	}
		   ...
		   struct spdk_sock_opts *opts)
{
	...
	sock = _posix_sock_connect(ip, port, opts, false, false, NULL, NULL);
	if (sock == NULL) {
		return sock;
	}
...
static struct spdk_sock *
posix_sock_connect_async(const char *ip, int port, struct spdk_sock_group_impl *group,
			 struct spdk_sock_opts *opts, spdk_sock_connect_cb_fn cb_fn, void *cb_arg)
{
	/* sock will be added to the sock group after it's connected */
		...
		struct spdk_sock_opts *opts)
{
	...
	sock = _posix_sock_listen(ip, port, opts, true);
	if (sock == NULL) {
		return sock;
	}
		 ...
		 struct spdk_sock_opts *opts)
{
	...
	sock = _posix_sock_connect(ip, port, opts, false, true, NULL, NULL);
	if (sock == NULL) {
		return sock;
	}
...
static struct spdk_sock *
ssl_sock_connect_async(const char *ip, int port, struct spdk_sock_group_impl *group,
		       struct spdk_sock_opts *opts, spdk_sock_connect_cb_fn cb_fn, void *cb_arg)
{
	/* sock will be added to the sock group after it's connected */

...
int devx_open_device(devx_ctx_t *ctx, struct ibv_device *device)
{
    ...
    if (!mlx5dv_is_supported(device)) {
        nvmx_error("devx is not supported on device %s", device->name);
        return -errno;
    }
    ...
    if (!ctx->ibv_ctx) {
        nvmx_error("devx: failed to open device %m errno=%d", errno);
        return -errno;
    }
    ...
    nvmx_debug("devx: opened device %s", device->name);
    return 0;
}
...
static struct devx_obj_handle *devx_obj_handle_alloc(devx_ctx_t *ctx)
{
    ...
    if (!h) {
        nvmx_error("devx: failed to allocate object handle");
        return NULL;
    }
...
err:
    nvmx_error("devx: failed to create object");
    free(h);
    return NULL;
...
fatal:
     ...
     * TODO: fix
     */
    nvmx_fatal("destructor allocation failure");
}
...
int devx_obj_destroy(struct devx_obj_handle *h)
{
    ...
    if (h->obj) {
        ...
        if (ret)
            nvmx_warn("FAILED to destroy tunneled object %p err %d dtor_length %d",
                      h, ret, h->dtor_in_len);
                                      ...
                                      int access, uint32_t *id)
{
    ...
    if (!umem) {
        nvmx_error("devx: failed to register umem: addr=%p size=%lld, flags=0x%x",
...
struct devx_obj_handle *devx_alloc_uar(devx_ctx_t *ctx, uint32_t *idx, void **addr)
{
    ...
    if (!uar) {
        nvmx_error("devx: failed to alloc uar");
        return 0;
    }
...
int devx_open_device(devx_ctx_t *ctx, struct ibv_device *device)
{
    nvmx_error("DEVX is not supported in this version of verbs");
    return -ENOTSUP;
}
